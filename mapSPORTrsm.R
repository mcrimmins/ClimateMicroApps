...
fileList<-fromJSON("https://geo.ndc.nasa.gov/SPoRT/modeling/lis/conus3km/geotiff/rsm_0-2m_percentile/?format=json")
...
fileName<-fileList$filename[which(fileList$date==max(fileList$date) & fileList$var=="rsm0-2m" & fileList$fileType=="float" & fileList$spatial=="wgs84.tif")]
mapDate<-fileList$date[which(fileList$date==max(fileList$date) & fileList$var=="rsm0-2m" & fileList$fileType=="float" & fileList$spatial=="wgs84.tif")]
  mapDate<-format(mapDate, "%m-%d-%Y")

download.file(paste0("https://geo.ndc.nasa.gov/SPoRT/modeling/lis/conus3km/geotiff/rsm_0-2m_percentile/",fileName),
              destfile = "temp.tif")
...
mapSMFig<-levelplot(mapSM, contour=FALSE, margin=FALSE, at=at,
                   ...
                   main=paste0("NASA-LIS 0-2m Relative Soil\nMoisture Percentile: ",mapDate))+
  layer(sp.polygons(kaibab_forest, col = 'black', lwd=1))

png("SPORT_INT2m_RSM.png", width = 10, height = 6, units = "in", res = 300L)
mapSMFig
dev.off()
